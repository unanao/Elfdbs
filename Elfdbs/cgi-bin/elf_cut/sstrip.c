...
#define	Elf_Ehdr	Elf32_Ehdr  //ELF Header page 8
#define	Elf_Phdr	Elf32_Phdr  //program header 24
#else
...
static int readelfheader(int fd, Elf_Ehdr *ehdr)
{
     ...
     */
    if (ehdr->e_ident[EI_DATA] != ELF_DATA)  //big is in the front
	return err("ELF file has different endianness.");
    if (ehdr->e_ident[EI_CLASS] != ELF_CLASS)//32 bit
	return err("ELF file has different word size.");

    /* Check the target architecture.
     */
    if (ehdr->e_machine != ELF_ARCH) //intel 32 or 64 bit
...
int main(int argc, char *argv[])
{
   ...
   // printf("Conten-type: text/html\n\n");
	...
	fd = open(*arg, O_RDWR);//if returned value fd =-1,open failed
	if (fd < 0) {
	    ferr("can't open");
	    ++failures;
	    continue;
